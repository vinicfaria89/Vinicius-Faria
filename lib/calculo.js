...
function calcularAmortizacaoCashSweep(ativo, curvas, iAnual) {
  const periodicidadeJuros = ativo.periodicidadeJurosCashSweep === 'semestral' ? 'semestral' : 'mensal';
  const jurosMeses = periodicidadeJuros === 'semestral' ? 6 : 1;
  const periodicidadeAmortizacao = ativo.periodicidadeAmortizacaoCashSweep === 'semestral' ? 'semestral' : 'mensal';
  const amortMeses = periodicidadeAmortizacao === 'semestral' ? 6 : 1;
...
function calcularDurationAtivo(ativo, calc) {
  ...
  if (ativo.cashSweep) {
    ...
    for (let i = 1; i < cs.saldoHistorico.length; i++) {
      const mes = cs.saldoHistorico[i].mes;
      const valorAmort = cs.saldoHistorico[i - 1].saldo - cs.saldoHistorico[i].saldo;
      const data = adicionarMeses(ativo.dataBase, mes);
      fluxos.push({ anos: diasEntre(ativo.dataBase, data) / 365, valor: valorAmort });
    }
    for (const j of cs.jurosHistorico) {
      const data = adicionarMeses(ativo.dataBase, j.mes);
      fluxos.push({ anos: diasEntre(ativo.dataBase, data) / 365, valor: j.valor });
    }
...
function calcularCarteira(ativos, curvas) {
  ...
  const cdiRefMedio = interpolar(duMedio, curvas.b3Pre.pontos); // DI Futuro (B3)
  const cdiAcumuladoPct = (Math.pow(1 + cdiRefMedio / 100, duMedio / 252) - 1) * 100;
