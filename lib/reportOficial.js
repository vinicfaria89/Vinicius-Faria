...
const CSS = `
  ...
  * { box-sizing: border-box; }
  body { margin:0; font-family:'Segoe UI', Arial, sans-serif; color:#2a2a1e; background:#e7e2d5; }
  ...
  .page-header { background:#2c2c1a; border-radius:14px; padding:14px 24px; display:flex; align-items:center; justify-content:space-between; margin-bottom:16px; }
  .page-header .titulo { color:#a3c96b; font-size:17px; font-weight:800; }
  .page-header .logo { color:#fff; font-size:19px; font-weight:800; letter-spacing:1px; }

  .page-footer { position:absolute; bottom:0; left:0; right:0; background:#2c2c1a; padding:10px 20px; display:flex; align-items:center; justify-content:space-between; gap:20px; }
  ...
  .metric-row::after { content:''; position:absolute; right:0; top:0; bottom:0; width:4px; background:#8fbf5a; }
  ...
  .sim-header .prod .tag-sweep { background:#a3c96b; color:#20300f; border-radius:4px; padding:1px 5px; font-size:7.5px; font-weight:800; text-transform:uppercase; letter-spacing:0.3px; display:inline-block; margin-left:4px; white-space:nowrap; }
  ...
  .nov-ganho-box { background:#a3c96b; color:#20300f; border-radius:14px; padding:14px 22px; margin-top:14px; display:flex; align-items:center; justify-content:space-between; gap:20px; page-break-inside:avoid; }
...
function montarPagina(conteudoHtml, numero, total, nomeAssessor, tituloPagina) {
  ...
  <div class="conteudo">
    <div class="page-header"><div class="titulo">${escapeHtml(tituloPagina || 'Carteira Simulada')}</div><div class="logo">GCB</div></div>
    ${conteudoHtml}
  </div>
...
async function montarRelatorioOficial({ cliente, dataBase, ativosInput: ativosInputRaw, curvas, templateType, docTitleSuffix, assessor, medirAlturasCards }) {
  ...
  const temCashSweep = ativosInput.some((a) => a.cashSweep);
  ...
  const fraseCupom = temCupomDistribuido
    ? ` Para os produtos com pagamento periódico de juros (${periodicidadeCupomLabel}), os juros são pagos ao investidor na periodicidade contratada e não reinvestidos — o valor futuro corresponde ao principal acrescido dos juros do período (sem capitalização dos juros). Quando o ativo não é isento, cada pagamento sofre retenção de Imposto de Renda pela tabela regressiva (22,5% até 180 dias; 20,0% até 360 dias; 17,5% até 720 dias; 15,0% acima de 720 dias), aplicada com base no prazo decorrido entre a data-base e a data daquele pagamento específico — por isso a alíquota efetiva tende a cair ao longo do tempo, pagamento a pagamento, em vez de um percentual único para toda a carteira.`
    : '';
  const fraseCashSweep = temCashSweep
    ? ` <h4>Cash Sweep</h4><p>Um ou mais ativos desta carteira possuem <b>Cash Sweep</b>: o principal é amortizado programadamente ao longo do prazo, em vez de devolvido de uma vez no vencimento, e os juros incidem sobre o saldo devedor remanescente — por isso o saldo investido e o valor de cada pagamento de juros diminuem com o tempo. Esta simulação não considera eventual aceleração da amortização além do cronograma programado; consulte os documentos da oferta para o cronograma efetivo.</p>`
    : '';
  const fraseReinvest = temReinvestimento
    ? ' Esta simulação assume que os juros dos ativos com opção de reinvestimento são reinvestidos pelo investidor a cada pagamento até o vencimento de cada ativo. Quando o ativo não é isento, o Imposto de Renda é descontado de cada pagamento de juros (pela tabela regressiva, conforme o prazo decorrido até aquele pagamento) ANTES de o valor líquido ser somado ao saldo reinvestido — o saldo cresce, portanto, sobre uma base já líquida de IR a cada período, e não por capitalização composta da taxa bruta.'
    : '';
  ...
  const fraseDebentureIsenta = temDebentureIsenta
    ? ' A isenção de Imposto de Renda aplicada às debêntures desta carteira pressupõe tratamento de debênture incentivada (Lei nº 12.431/2011); essa condição deve ser confirmada nos documentos da oferta antes de qualquer decisão de investimento — debêntures comuns (não incentivadas) são tributadas pela tabela regressiva, como qualquer outro ativo de renda fixa.'
    : '';
  const fraseIR = (todosIsentos
    ...
    : ' Os valores desta simulação são apresentados líquidos de Imposto de Renda: CRI e CRA são isentos de IR para pessoa física; os demais ativos são tributados pela tabela regressiva do IR sobre renda fixa (22,5% até 180 dias; 20,0% até 360 dias; 17,5% até 720 dias; 15,0% acima de 720 dias).') + fraseDebentureIsenta;
  ...
  // já produzidos por calcularCarteira (nada é recomputado aqui).
  const fraseMetodologiaCdi = `<h4>Comparação com o CDI (≈${Math.round(carteira.pctDoCdi)}% do CDI)</h4>
    <p>O comparativo "≈${Math.round(carteira.pctDoCdi)}% do CDI" mede o retorno líquido total da carteira em relação ao retorno que um investimento a 100% do CDI teria entregado no mesmo prazo médio ponderado desta carteira (${Math.round(carteira.prazoMedioDias)} dias corridos, ≈${Math.round(carteira.duMedio)} dias úteis). Para esse prazo, a taxa de referência interpolada na curva DI Futuro (B3) na data-base é de ${pct2(carteira.cdiRefMedio)}% a.a., equivalente a um CDI acumulado de ${pct2(carteira.cdiAcumuladoPct)}% no período. Cálculo: % do CDI = (Retorno líquido da carteira ÷ CDI acumulado no período) × 100 = (${pct2(carteira.retornoLiquidoPct)}% ÷ ${pct2(carteira.cdiAcumuladoPct)}%) × 100 ≈ ${Math.round(carteira.pctDoCdi)}%.</p>`;

  // Explica por que o "% do CDI" pode aparecer abaixo de 100% mesmo quando a carteira tem ativos
  // indexados ao (ou acima do) CDI — dúvida recorrente do cliente, que tende a interpretar o número
  // como "renderam menos que o CDI" quando na verdade é um efeito de IR: o CDI acumulado usado na
  // comparação é BRUTO (sem imposto), enquanto o retorno da carteira já é LÍQUIDO (IR já descontado).
  const fraseCdiAbaixoDe100 = `<h4>Por que o % do CDI pode ficar abaixo de 100%, mesmo com ativos indexados ao CDI?</h4>
    <p>O CDI acumulado usado nesta comparação (${pct2(carteira.cdiAcumuladoPct)}%) é um valor <b>bruto</b>, sem desconto de Imposto de Renda — é o que um investimento hipotético a 100% do CDI renderia antes de qualquer tributação. Já o retorno da carteira (${pct2(carteira.retornoLiquidoPct)}%) é <b>líquido</b>, com o IR regressivo já descontado de cada ativo. Por isso, mesmo uma debênture ou CDB que pague exatamente 100% do CDI (ou um spread sobre ele) aparece abaixo de 100% nesta métrica depois de tributada — não é um sinal de que o produto rendeu menos que o CDI de mercado, e sim o efeito natural de comparar um número líquido de IR contra um número bruto. Ativos isentos de IR (como CRI/CRA/LCI/LCA) tendem a aproximar-se mais de 100%, ou ultrapassá-lo, justamente por não sofrerem esse desconto.</p>`;

  // Materiais de divulgação de produto (ex.: catálogo/site da GCB) costumam anunciar o "% do CDI"
  // por uma conta mais simples — taxa BRUTA contratada dividida pelo CDI corrente do dia, sem juros
  // compostos e sem IR — pensada pra comparação rápida entre produtos, não pra projetar retorno.
  // Esse relatório usa uma conta mais rigorosa (ver caixa acima), então os dois números legitimamente
  // não batem; sem essa nota, o cliente pode achar que um dos dois está errado.
  const fraseCdiConvencaoDiferente = `<h4>Por que esse % do CDI pode ser diferente do anunciado no material do produto?</h4>
    <p>Materiais de divulgação (ex.: catálogo de produtos) costumam mostrar o "% do CDI" de forma simplificada: a taxa <b>bruta</b> contratada do papel dividida pelo CDI <b>do dia</b> (sem capitalização composta e sem desconto de IR) — uma régua rápida para comparar produtos entre si, não uma projeção de retorno. Este relatório usa uma conta mais rigorosa: o retorno <b>líquido de IR</b> dividido pelo CDI <b>acumulado (composto)</b> até o vencimento de cada ativo — por isso os dois números, ambos corretos dentro da sua própria metodologia, não coincidem. Em prazos mais longos, essa diferença tende a ser maior, porque o CDI acumulado composto cresce mais que uma simples multiplicação linear da taxa anual pelo prazo.</p>`;
  ...
  const fraseDuration = divergeDuration
    ...
    <p>O <b>Prazo Médio até o Vencimento</b> é a data contratual em que o capital retorna integralmente — a data marcada no papel. A <b>Duration</b> é quando, na prática, o investidor já recebeu de volta a maior parte do valor investido, porque leva em conta os cupons e amortizações recebidos ao longo do caminho, não só o pagamento final. Como esta carteira tem produto(s) com pagamento de juros ou amortização antes do vencimento, parte do retorno chega antes — por isso a Duration (≈${carteira.durationAnos.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} anos) é menor que o Prazo Médio (≈${carteira.prazoMedioAnos.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} anos): na prática, o capital fica exposto ao risco de prazo por menos tempo do que a data de vencimento sugere.</p>`
    : `<h4>Prazo Médio até o Vencimento x Duration</h4>
    <p>Nesta carteira, o <b>Prazo Médio até o Vencimento</b> e a <b>Duration</b> coincidem: como o(s) produto(s) só devolve(m) o capital integralmente no vencimento (sem pagamento de juros ou amortização antes disso), não há antecipação de fluxo de caixa a ser considerada.</p>`;
  ...
  const rentAnualizadaPct = (Math.pow(1 + carteira.retornoLiquidoPct / 100, 365 / carteira.prazoMedioDias) - 1) * 100;
  ...
  <div class="stats-extra-card">
    ...
    <div class="stat-item">
      ...
      <div class="stat-val">≈${pct2(rentAnualizadaPct)}% a.a. <span class="stat-sub">taxa de crescimento anual composta sobre o prazo médio ponderado</span></div>
    </div>
  </div>`;
  ...
  const simCardsArr = carteira.resultados.map((r, i) => {
    ...
    const isCashSweep = !!a.cashSweep;
    const distribuiCupom = !isCashSweep && a.pagaCupomMensal && !a.reinvestir;

    // Ativos com cupom distribuído (ou Cash Sweep) usam um gráfico de fluxo (escada) que separa
    // visualmente o principal da renda recebida; ativos bullet/reinvestidos mantêm a linha diagonal de
    // capitalização composta. Cash Sweep tem seu próprio gráfico (saldo devedor caindo + juros somando).
    ...
    const ehAmortSemestral = isCashSweep && c.cashSweep.periodicidadeAmortizacao === 'semestral';
    const periodoAmortLabel = ehAmortSemestral ? 'semestral' : 'mensal';
    ...
    const legendaGrafico = isCashSweep
      ...
      ? `<div class="row"><span class="dot" style="background:#b0ac92"></span>Principal (não capitalizado)</div>
            ...
            <div class="row"><span class="dot" style="background:#9a9a8c"></span>100% CDI (${refCdiIndiv.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}% a.a.)</div>`
      : `<div class="row"><span class="dot" style="background:#8fbf5a"></span>${escapeHtml(categoriaLabel(a.tipoProdutoLabel))}</div>
            <div class="row"><span class="dot" style="background:#9a9a8c"></span>100% CDI (${refCdiIndiv.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}% a.a.)</div>`;

    const fluxoPagamentoLabel = isCashSweep
    ...
    return `<div class="sim-card">
      <div class="sim-header">
        <div class="prod"><div class="cat">${escapeHtml(categoriaLabel(a.tipoProdutoLabel))}${a.cashSweep ? ' <span class="tag-sweep">Cash Sweep</span>' : ''}</div><div class="nome">${escapeHtml(a.nome)}</div></div>
...
<div class="page-oficial" style="overflow:visible;">
  <div class="conteudo">
    <div class="page-header" id="m-header"><div class="titulo">Carteira Simulada</div><div class="logo">GCB</div></div>
...
<div class="page-oficial" style="overflow:visible;">
  <div class="conteudo">
    <div class="page-header" id="m-header"><div class="titulo">Novação de Debêntures</div><div class="logo">GCB</div></div>
