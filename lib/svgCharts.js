...
// A primeira cor é o verde claro oficial da GCB (marca/ação) — as demais são tons de apoio pra
// diferenciar categorias quando há mais de um ativo/produto no mesmo gráfico (o guia de marca não
// define uma paleta categórica completa, só as 3 cores primárias).
const PALETA_BASE = ['#A8D18B', '#5b7d3a', '#9c5b5b', '#4a7a96', '#c9a227', '#7a5a96'];
...
function gerarLinhaComparativaSVG({ vi, vf, vfCdi, dataVencFmt }) {
  // top/bottom mais espaçados (eram 26/108, viewBox 148) — o card do gráfico, na Carteira Simulada,
  // estica em altura pra acompanhar a coluna de métricas ao lado (que pode ter várias linhas, ex.:
  // Cash Sweep); como o SVG só respeita sua própria proporção largura/altura (não "estica" sozinho
  // pra preencher o card), uma proporção mais alta aqui evita o gráfico ficar visualmente pequeno e
  // apertado no topo do card, com espaço em branco sobrando embaixo.
  const top = 26;
  const bottom = 142;
  const xIni = 42;
  const xFim = 418;
  ...
  return `<svg width="100%" viewBox="0 0 460 182" class="linha-comparativa">
    ...
    <line x1="${xIni}" y1="${yIniAtivo.toFixed(1)}" x2="${xFim}" y2="${yFimAtivo.toFixed(1)}" stroke="#A8D18B" stroke-width="2.2"/>
    <circle cx="${xIni}" cy="${yIniAtivo.toFixed(1)}" r="3" fill="#A8D18B"/>
    <circle cx="${xFim}" cy="${yFimAtivo.toFixed(1)}" r="3" fill="#A8D18B"/>
...
function gerarFluxoCupomSVG({ vi, cupomLiquidos, vfCdi, dataVencFmt }) {
  // Ver nota em gerarLinhaComparativaSVG sobre por que top/bottom cresceram (eram 26/108).
  const top = 26;
  const bottom = 142;
  const xIni = 42;
  const xFim = 418;
  ...
  return `<svg width="100%" viewBox="0 0 460 182" class="linha-comparativa">
    ...
    <polygon points="${areaPontos}" fill="#A8D18B" fill-opacity="0.16" stroke="none"/>
...
function gerarFluxoCashSweepSVG({ vi, saldoHistorico, jurosHistorico, mesesTotais, vfCdi, dataVencFmt }) {
  ...
  return `<svg width="100%" viewBox="0 0 460 182" class="linha-comparativa">
    ...
    <polygon points="${areaJurosPontos}" fill="#A8D18B" fill-opacity="0.16" stroke="none"/>
