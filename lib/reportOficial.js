...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .sim-chart-card { flex:1; background:#fff; border-radius:8px; padding:11px 14px; }
  .sim-chart-legend { display:flex; flex-wrap:wrap; gap:5px 16px; font-size:8.5px; margin-top:6px; color:#5a5847; }
  ...
  .sim-chart-tag { display:inline-block; font-size:7.5px; font-weight:800; text-transform:uppercase; letter-spacing:0.3px; border-radius:4px; padding:2px 7px; margin-bottom:8px; }
...
async function montarRelatorioOficial({ cliente, dataBase, ativosInput: ativosInputRaw, curvas, templateType, docTitleSuffix, assessor, medirAlturasCards }) {
  ...
  <div class="stats-extra-card">
    ...
    <div class="stat-item">
      <div class="stat-lbl">Duration (Fluxo de Caixa)</div>
  ...
  const simCardsArr = carteira.resultados.map((r, i) => {
    ...
    const jmAtivoRow = (distribuiCupom || isCashSweep)
      ? `<div class="metric-row jm"><span class="lbl">Juros ${ehSemestral ? 'Semestrais' : 'Mensais'} (líq.)<span class="lbl-sub">1º pagamento</span></span><span class="val">${brl2(c.cupomLiq)} ${porPeriodoLabel}</span></div>`
      : '';
...
<div class="page-oficial" style="overflow:visible;">
  ...
  <div class="page-footer">
    <div class="disclaimer"><div class="autor">medição</div></div>
